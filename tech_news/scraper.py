...
def get_tech_news(amount):
    ...
    while len(resp_news) < amount:
        html_page = fetch(url)
        news_list = scrape_novidades(html_page)

        for news_url in news_list:
            next_page = fetch(news_url)
            page = scrape_noticia(next_page)
            resp_news.append(page)
            if len(resp_news) == amount:
                break
            url = scrape_next_page_link(html_page)
    create_news(resp_news)
    return resp_news
